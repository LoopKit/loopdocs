...
!!! danger "API Key vs APN Key"
    If you build with the *GitHub* [Browser Build](../gh-actions/gh-overview.md), you may notice <span>the <code>Application Programming Interface (API)</code> key</span> has the same type of format as <span>the <code>Apple Push Notification (APN)</code> key</span>. The keys for both purposes are of type `p8`, but should not be confused.
...
### <span translate="no">LoopCaregiver</span>   ![icon for LoopCaregiver app](img/lcg-icon.jpg){width="50"}
...
A build script is available to assist in building *LoopCaregiver*. This should be straightforward for anyone who has previously built &nbsp;*<span translate="no">Loop 3</span>*&nbsp; using the script. You may also choose to build it with the GitHub build method as documented on the Browser Build: [Other Apps](../gh-actions/gh-other-apps.md) page.
...
* On the Looper&#39; phone:
    ...
    * !!! tip "Pro-tip"
           ...
           You can then add view the QR code in *LoopCaregiver* without bothering your Looper.
...
You can also use <span>the `LoopCaregiver -> Settings` screen</span> to modify:
