...
This page is for the "advanced" user. Well, maybe there is a better word for it. Adventurous? Curious? But, if you are a newbie and not into crazy stuff...everything in here is completely and totally just optional. Using or not using LoopWorkspace does not affect your looping nor access to Loop. This page is an efficient way to answer questions about a topic that takes a lot of words to answer.

!!! info "Build Select Script"
    Recently a new method using the [Build Select Script](https://www.loopandlearn.org/build-select/) became available for stable or released versions of Loop. It does use the LoopWorkspace method, but you don't need to know the details that are on the rest of this page.  Just follow the script and the accompanying graphic.

    * Using the build select method is faster than a zip build
    * An appropriately named folder that includes the date and time of the download is automatically created
    * If you later want to delete one of those folder, look under Downloads/LoopBuild for folders


There's another way of getting Loop code to your computer called "git". Git is a system of "distributed version control" that allows remotely (as in not located in same place) collaborating people to work on one project and still track their changes to the same place. For example, if I sent 5 people one document to proof-read at the same time...it is quite possible that the edits I will get back from those 5 people would conflict with each other. Bob may have entirely deleted a sentence while Mary would have added words to that sentence. Git lets these remotely collaborating people deal with "resolving conflict" between versions more easily and merging suggestions (pull requests) into a coordinated space.
...
But yes, git commands take awhile to properly use. And they are not plain english friendly. Or any kind of friendly really, at least not to very many people. Therefore, we have emphasized the downloading of a zip file or using the build select script to make you not have to use git.

## Why this page?

So why am I about to tell you about a Loop repo that involves git? The answer is because LoopWorkspace is a pretty nice feature that you may run across if you are testing new features. And if you do, most of the questions you may have are answered on this page.

!!! info "You can still download zips"

    You do not have to use LoopWorkspace. Just to be clear, everything still works like it used to. LoopWorkspace is just a different option to save the amount of work that you would have to do to test new features before they are released. Depending on what you'd like to do, it may be a good option. Usually that "good option" is if you are trying new code that involves edits to the Loop frameworks.

## What is LoopWorkspace?

I'm going to retread some info that you may have read in the [Branch and Versions FAQs](../faqs/branch-faqs.md) page, but is really important to this discussion on LoopWorkspace. Loop developers own an account in GitHub called [LoopKit](https://github.com/LoopKit).  Within that account, the developers have several "repositories" that support Loop in particular. A repository is like a book...let's think of it like a cookbook for now. Within the LoopKit account, there are repositories for Loop itself, LoopDocs, and various other supporting "frameworks" that are helper repositories for Loop to build correctly. For example, Loop's repo has a lot of the info about the app itself; the outward facing things that you interact with. How information is put to you and taken in from you...that's in Loop repository code. But, there's more than just a user interface for Loop. Loop has to do a lot of complex work like Bluetooth communications, algorithm math, pump communications, etc. The Loop app has help from frameworks to do those other parts. CGMBLEkit for some of the transmitter parts of Loop, RileyLink_ios for the pump managers (talking to the pumps and decoding their information), LoopKit for the algorithm about carbs and insulin curves, etc.

When you build Loop, in the background, Loop pulls those other frameworks into the build process using "carthage".  The carthage utility is like a personal shopper. You give it a shopping list (the cartfile in Loop code is that shopping list) and it goes and fetches that for you during the build process. The cartfile shows where the various frameworks are (exactly which GitHub account) and which branch to use. The graphic below is an example of what the cartfile looks like for an older version of Loop.  (Look on github for the most recent version).
...
So, you will need to edit that "branch-name" before using the command so that you are getting started with the branch you want. For example, to test dev (which is under development and has some cool new architecture and features), you would type:

`git clone --branch=dev --recurse-submodules https://github.com/LoopKit/LoopWorkspace`

Skip ahead to [Where does the clone go?](#where-does-the-clone-go) if you want to use finder.

If you want to start the build from the command line, the next step is shown below and skip ahead to [Building LoopWorkspace](#building-loopworkspace)
...
Once you are in LoopWorkspace opened in Xcode, everything is pretty similar for building with only two notable exceptions. You need to click on that blue Loop folder to see the signing targets, and you need to change the build scheme to the left of your phone to "Loop (Workspace)" in order to build properly. With the latest version of LoopKit/LoopWorkspace, might be different for other versions of LoopWorkspace, you can enter your Apple Developers ID in the LoopConfigOverride.xcconfig file that now appears in the top of the folder list (not shown in this graphic).  Doing this automatically signs the 4 targets.
...
When you want to update your Loop app using LoopWorkspace, you'll need to use  `git pull --recurse` command while in the LoopWorkspace directory in Terminal. If this does not seem to get the updates you expected, use the [Stay up-to-date with dev branch](#stay-up-to-date-with-dev-branch) method.

If you have a signing team filled in, you may get a conflict on the command...so either unsign before the git pull or do a `cd Loop`, `git stash`, `cd ..` starting from the LoopWorspace directory and try again.
...
## Stay up-to-date with dev branch

LoopWorkspace and associated repositories for the dev branch are under active development.  The `clone` command points to the dev branch of LoopWorkspace, but downloads a detached `HEAD` for each of the submodules, which appear as subfolders under the LoopWorkspace folder.  (Search for `git detached HEAD` online to learn more). If you want to incorporate a change that has been added to one of the submodules but has not been updated in the LoopWorkspace dev branch yet, you need to take the following steps for the desired repository.

Example commands to attach a given submodule to the dev branch instead of the detached `HEAD` are given below for the most likely repositories to be modified. If these commands do not make sense to you, please do a search on using git and understand them before proceeding. Note that if you do this right after a fresh clone, the detached `HEAD` will be pointing to the most recent version of the dev branch for that repository so you will see the `Already up to date`.  Note that if you have followed these steps, and then use `git pull --recurse` command in the LoopWorspace folder, all of the submodules will be reset to the detached `HEAD` and no longer point to the dev branch.

```
cd Loop; git checkout dev; git pull; cd ..

cd LoopKit; git checkout dev; git pull; cd ..

cd rileylink_ios; git checkout dev; git pull; cd ..
```

For example, the zulipchat #github stream shows that a commit has just been added to the dev branch of Loop.  

* In order to add that commit to your clone on your computer:
    * `cd Loop; git checkout dev; git pull; cd ..`
    * If you have made modifications in your folder of Loop and git refuses to do the pull, you can do this instead:
    * `cd Loop; git stash; git checkout dev; git pull; git stash pop; cd ..`
    * if there are conflicts, you'll need to handle them manually
